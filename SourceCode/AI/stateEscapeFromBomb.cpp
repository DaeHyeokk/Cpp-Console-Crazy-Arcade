...
void StateEscapeFromBomb::moveToSafeSpot()
{
	...
	while (!posQ.empty())
	{
		...
		// 현재 위치가 안전한 좌표라면 해당 좌표까지 이동 가능한지 확인
		if (enemy.getDangerMapValue(pos.y, pos.x) >= SAFE_BOMB_TIME)
		{
			// (이동 딜레이 * 목적지까지 칸수)까지의 시간이 폭발 남은 시간보다 짧다면 
			// 해당 경로와 현재까지 발견한 가장 안전한 경로와 비교한다
			if (moveDelay * depth < explodeRemainTime)
			{
				// 이전 탐색에서 발견한 가장 안전한 경로보다 더 안전한 경로라면 
				// 가장 안전한 경로를 현재 경로로 갱신한다
