		...
		if (!objects.empty())
		{
			for (auto& object : objects)
			{
				if (object && object->activeSelf())
				{
					...
					{
						Character* target = static_cast<Character*>(object);
...
void Character::placeBomb()
{
	...
	if (elapsedMS > PLACE_BOMB_DELAY)
	{
		...
		if (isCanPlaceBomb())
		{
			board.setObject(std::make_unique<Bomb>(position, board, power, *this));
			plantBombsCount++;
...
void Character::pickupItem(unique_ptr<Item> pickItem)
{
	...
	if (pickItem->getItemCategory() == ItemCategory::CONSUME)
		takeConsumeItem(std::move(pickItem));
