...
void Board::setBoard(std::vector<std::vector<int>>& newBoard)
{
	...
	for(int y = 0; y < maxY; y++)
		for (int x = 0; x < maxX; x++)
		{
			...
			case BLOCK:
				objects.push_back(std::move(std::make_unique<Block>(pos, *this)));
				objects.back()->draw();
				break;

			case DISTRUCTIBLEBLOCK:
				objects.push_back(std::move(std::make_unique<DistructibleBlock>(pos, *this)));
				objects.back()->draw();
				break;
			}
		}

	flamePool = std::make_shared<FlamePool>(*this);
	flamePool->init();
...
{
	std::unique_ptr<Character> player = std::make_unique<Player>(Position{ 1,5 }, *this);
	std::unique_ptr<Character> enemy = std::make_unique<Enemy>(Position{ 11,13 }, *this, player.get());
	setObject(std::move(enemy));
	setObject(std::move(player));
...
std::vector<GameObject*> Board::getObject(Position position)
{
...
std::unique_ptr<GameObject> Board::getObjectUniquePtr(GameObject* gameObj)
{
	for(auto& object : objects)
		if (object && object.get() == gameObj)
		{
			std::unique_ptr<GameObject> retObject = move(object);
			return retObject;
		}
...
void Board::setObject(std::unique_ptr<GameObject> gameObj)
{
	...
	for(auto& object: objects)
		if (object)
		{
			...
			else
			{
				...
				std::unique_ptr<GameObject> moveObj = std::move(object);
				rawPtr->returnObject(std::move(moveObj));
				// object->returnObject(std::move(object)); 이 코드와 의미가 같음.
			}
		}
